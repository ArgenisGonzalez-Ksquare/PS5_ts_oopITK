...
import {ListInfo } from "./interface/interface" //IMPORT THE INTERFACE FOR THE FETCH
...
const initial =  function initialState(resp) {
    for (let index = 0; index < resp.length; index++) {
        document.querySelector(`.tittle${index}`).innerHTML = resp[index].title; // the titles of the cards
        document.querySelector(`.current${index}`).innerHTML = '- Hrs';
        document.querySelector(`.preovius${index}`).innerHTML = '-';
    }
}
...
.then((resp:ListInfo) => { //the information will be recieve according to the interface type that we have created
    ...
    dailyB.addEventListener("click", () => {
        for (let index = 0; index < resp.length; index++) {
            ...
            document.querySelector(`.preovius${index}`).innerHTML = resp[index].timeframes.daily.previous;
        }
    })
    weeklyB.addEventListener("click", () => {
        for (let index = 0; index < resp.length; index++) {
            document.querySelector(`.current${index}`).innerHTML = resp[index].timeframes.weekly.current + 'Hrs';
            document.querySelector(`.preovius${index}`).innerHTML = resp[index].timeframes.weekly.previous;
        }
    })
    monthlyB.addEventListener("click", () => {
        for (let index = 0; index < resp.length; index++) {
            document.querySelector(`.current${index}`).innerHTML = resp[index].timeframes.monthly.current + 'Hrs';
            document.querySelector(`.preovius${index}`).innerHTML = resp[index].timeframes.monthly.previous;
     }
    })
})
.catch((err) => {
